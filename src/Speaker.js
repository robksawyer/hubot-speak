...
module.exports = {
  ...
  WordException: function (dictionary) {
 ...
 * @return String    Word
 */
  getLastWord : function (statement, start) {
    var previousSpace;
   ...
   */
  requirementFailed: function (message) {
   ...
   */
   SpeechLibrary: function (name, token, description, list, action, exceptions) {
   ...
   * @param    WordException    A WordException SpeakComponent
   */
  Mood: function (name, influencers, punctuations, emoticons, filter, exceptions) {
      this.name = name || requirementFailed('Name is required');
   ...
   */
  Speak: function (libraries, moods, statements, segments) {
      // SpeechLibraries
      this.libraries = libraries || [
          ...
          new SpeechLibrary('emotives', 'emo', 'n-time self-referencing',
              ...
              function (statement, position, mood, speak) {
                ...
                var lastWord = getLastWord(statement, position);
